...
async def ensure_indexes():
    ...
    await db.payments.create_index("payment_channel")
    await db.payments.create_index("collector_admin_id")
    await db.payments.create_index("remittance_id")
    await db.payments.create_index([("collection_number", 1)], unique=True, sparse=True)
    await db.admin_remittances.create_index("remittance_number", unique=True)
    await db.admin_remittances.create_index([("admin_id", 1), ("status", 1)])
    await db.admin_remittances.create_index([("lender_id", 1), ("status", 1)])
    await db.admin_remittances.create_index("created_at")
